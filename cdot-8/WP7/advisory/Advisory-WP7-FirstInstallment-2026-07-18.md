...
> **ERRATUM (worker, 2026-07-19)**: §1's "$kR_h\sim6$ at the first
> acoustic peak" table entry and the "several-percent window — must be
> carried" conclusion drawn from it are **wrong by four to six orders of
> magnitude** — a $\Lambda$CDM comoving-horizon value was never actually
> checked against cdot-8's own $R_h(z)$ trajectory. Conceded in full as
> advisor error #8
...
> low-$\ell$ signature relocates to the coupled-era growth history and a
> late-time $\ell\lesssim10$ remnant; it is not simply erased. Everything
> else in this document (the import, the $\delta\mathcal N\neq0$
> resolution, the $\Omega_s$ zero-crossing finding) is unaffected and
> stands.
