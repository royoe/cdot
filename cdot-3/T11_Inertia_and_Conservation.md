...
$$L = mvr = \text{const}.$$
With $m$ invariant, $v = (GM/r)^{1/2}$ (force balance), $G \propto c^{-2}$:
$$L^2 = m^2v^2r^2 = m^2 \cdot \frac{GM}{r} \cdot r^2 = m^2 GMr \propto c^{-2} \cdot r.$$
Constant $L$ requires $r \propto G^{-1} \propto c^2$. Orbits expand (T9).

**The energy of the orbit is NOT conserved**: the total orbital energy
$E = -GMm/(2r) \propto -c^{-2}/c^2 \propto -c^{-4}$ changes as $c$ grows. This is
precisely what the symmetric flip requires — energy changes, angular momentum does not.
The orbit gains energy (in the sense that $|E|$ decreases, the orbit becomes less
bound) while angular momentum is conserved. This is internally consistent.
...
$$\frac{GM_u}{R_u c^2} = \frac{G \cdot \tfrac{4}{3}\pi R^3 \rho}{R c^2} \propto \frac{c^{-2} \cdot R^2}{c^2} \propto c^{-4} R^2.$$
This is not preserved over time; it varies as $c^{-10/3}$ and is only approximately $1/2$
today by coincidence. Sciama's literal Machianism is incompatible with invariant mass.
