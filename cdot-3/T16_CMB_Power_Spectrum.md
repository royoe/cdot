...
horizon dynamics with $G \propto c^{-2}$ and $M \propto R^3$:
$$\frac{r_s}{R} \propto R^{2-4n},$$
where $n$ is the counting-law exponent ($c \propto R^n$). For $n > 1/2$ (which
includes the volume law $n=3$, surface law $n=2$, and S′ $n=2/3$), this ratio
...
barrier into the PBH-formation mechanism. Crucially, this is **generic to any
counting law** (all undergo the $r_s/R \sim 1$ crossover), so PBH formation does
