...
| Element | Status |
|---|---|
| Premise 1 (static geometry) | Unchanged from cdot-4 |
| **Premise 2 (connectivity counting, two phases via a percolation transition)** | **New for cdot-5; see T23 for the full model.** Post-percolation ($z<z_*\approx1.2$) branch $c\propto e^{R/L}$ fits DESI DR2 clean galaxy bins alone at $\chi^2=13.2/7$; the full two-phase law (with a subcritical/occupancy branch above $z_*$) fits all six DESI bins at $\chi^2=6.8/8=0.85$, resolving the high-$z$ (QSO/Lyα) failure of the single-phase law. Rests on several named, undecided assumptions (supercriticality; endpoint-$1/L$ recruitment; mean-field independence; the percolation condition itself) — see T23. |
