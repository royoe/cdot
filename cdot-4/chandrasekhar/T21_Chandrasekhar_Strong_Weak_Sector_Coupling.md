...
*Status: new topic, proposed for merge. Numbered T21 provisionally (T19 confirmed
in progress elsewhere; T20 is this session's white-dwarf-population document) — please
verify T21 is free before merging, as T19 turned out not to be. Cross-references T4
(Chandrasekhar-mass candle systematic), T7 (fine-structure constant), T8 (invariant
$G$), T13 (BBN), T20 (white dwarf age ceiling). Session dates: 2026-07-01 (initial);
2026-07-01 (revision — Part 4 restructured around plasmon decay); 2026-07-01 (Part 5
added — plasmon decay turned into an actual WD cooling-age correction, with direct,
favorable implications for T20).*
...
sectors couple to $c$. This document maps out exactly where that happens, does the one
piece of the calculation that closes cleanly, and identifies where the genuinely open
piece is.

**Bottom line in advance:** one candidate channel (electron-capture instability) is
now closed — it is a small, bounded correction *no matter what* is assumed about
nuclear-sector $c$-scaling, so it cannot be where the overcorrection gets resolved. A
second, composition-independent channel — **plasmon decay**, the dominant neutrino
cooling process in degenerate cores — turns out to be a second *closed* sub-calculation:
its rate scales as $c^{-1}$ at fixed density, using only the two couplings already
fixed elsewhere in this document ($\alpha$, $G_F$). Turning that rate result into an
observable consequence (Part 5) gives the genuinely load-bearing finding of this
document: **standard white-dwarf cooling ages systematically overstate the true age**,
by an amount that plausibly explains T20's marginal ceiling violations outright and
meaningfully reduces its larger ones. The narrower, composition-specific Urca-pair
channel originally proposed is demoted to a secondary note. A concrete, derived
replacement for T4's ad hoc weak-decay-rate placeholder is also proposed as a byproduct.
...
| **Weak sector** ($G_F$, decay rates) | **unfixed** — T4 provisional placeholder: "decay rate $\propto c^2$, energy per decay $\propto c^2$" | T4 (flagged open) |
| **Strong sector** (nuclear binding, hadron radii, $\alpha_s$-analogue) | **unfixed** — not addressed anywhere yet | T13 (flagged open) |

The Chandrasekhar-mass problem sits exactly on top of the two unfixed rows. This
document's job is to make progress on both, specifically as they bear on $M_\text{Ch}$.
...
- **O/Ne/Mg-core white dwarfs** (the progenitors of most observed *ultra-massive* WDs,
  ...
  2017; Wu & Wang 2018 composition; the "AIC mass" is usually quoted around
  $1.37$–$1.39\,M_\odot$, a few percent below the idealized $1.4\,M_\odot$, though
  treatments differ — one recent EOS-dependent study quotes $1.48\,M_\odot$).
- **C/O-core white dwarfs** (classical single-degenerate SN Ia progenitors):
  ...
  $\log\rho\approx9.1$; Iben 1978, Piro & Bildsten 2008, Schwab et al. 2016). Whether and
  when ignition actually happens is a competition between gravitational compressional
  heating and Urca-process neutrino cooling.

Both processes bring in exactly the two unfixed sectors from Part 0: **nuclear
binding-energy differences (Q-values)** — strong + electromagnetic physics — and **weak-
interaction rates** — the Fermi constant $G_F$.
...
This is not the same as T4's placeholder, and it changes the derived decay-rate scaling.
Restoring $\hbar,c$ in the standard (muon, or any comparable 5th-power) weak decay-rate
formula by dimensional analysis:
...
**Proposed correction to T4: weak decay rate $\propto c^4$, not $c^2$.** The "energy per
decay $\propto c^2$" half of T4's placeholder is confirmed (and now derived); the "decay
rate $\propto c^2$" half should be revised to $c^4$.
...
rate-independent (T4's own words: "This is rate-independent and width-independent — the
most trustworthy statement"). What *is* affected is the still-open, $q$-gated
peak-luminosity/light-curve-timescale route, and the BBN weak-rate input in T13 (see
below) — both currently flagged as needing exactly this number.
...
heating and neutrino cooling. Two cooling processes were considered in this
investigation; they turn out to be very different in tractability.
...
- **Those two couplings are exactly the ones already fixed in this investigation.** The
...
fixed density) and applying the model's scalings — $\alpha$ invariant (T7, already
established) and $G_F,\hbar$ invariant (Part 3's proposal) — gives $E_p\propto c$, and
therefore:
$$\boxed{\Gamma_{\gamma^*\to\nu\bar\nu} \;\propto\; \frac{c^5}{c^6} = c^{-1}}$$

**Yes — the plasmon decay rate is affected by a varying $c$, and the answer is clean
and unambiguous given the couplings already fixed elsewhere in the model.** At fixed
physical density, plasmon-decay neutrino cooling was *more* efficient by a factor
$c_0/c(t)$ in the past (small $c$), not less — the opposite sense from the crude
intuition that "everything was weaker when $c$ was smaller." This follows directly
from $E_p\propto c$ (a fixed-momentum, relativistic energy grows with $c$, exactly as
$E=pc$ demands) combined with the steep 5th-power energy dependence of the weak-decay
phase space, only partially offset by the $c^6$ in the denominator from the
dimensional restoration.

**Why this is the more promising candidate than Part 4b's Urca pairs.** This result
rests on exactly two inputs — invariant $\alpha$ (already firmly established, T7) and
invariant $G_F$ (this document's Part 3 proposal) — with no unresolved nuclear-structure
uncertainty and no composition dependence. It is a genuinely closed sub-calculation, in
the same sense that Part 2's electron-capture result was closed, except here the answer
is a definite power ($c^{-1}$) rather than a bounded small correction, because plasmon
decay is not "how close to an asymptote" physics — it is one term in the heating/cooling
competition that decides where ignition or collapse happens at all.

**Qualitative implication (not yet a full answer).** Because cooling was relatively
*more* effective at fixed density in the past, a compressing core in the deep past would
need to be pushed to a *higher* density before compressional heating could win against
plasmon cooling — pushing the effective ignition point closer to (not further from) the
electron-capture/AIC threshold at every epoch. Taken at face value this suggests two
possible consequences, either of which would be a genuine, checkable prediction of the
model: (i) the single-degenerate SN Ia channel becomes progressively suppressed relative
to AIC at earlier times/higher redshift as cooling increasingly wins the race, which
would be a testable statement about the evolution of the SN Ia delay-time-distribution
and progenitor mix with redshift; or (ii) the SNe Ia that do still occur at early times
ignite at higher density and hence closer to the idealized $M_\text{Ch}(c)$ at their
epoch than SNe Ia do today, which would make the *realized* explosion mass track the
idealized $c^{3/2}$ curve **more** tightly in the past, if anything reinforcing rather
than relieving T4's overcorrection concern. **This is a direction, not a verdict** — it
requires the compressional-heating side of the competition (not derived here) before
either consequence can be stated quantitatively. That heating-rate calculation is now
the single best-defined next step in this whole investigation.

### Part 4b — Carbon Ignition vs. Urca Pairs (secondary note)

The Urca-pair-specific analysis from the previous version of this document is retained
below for completeness, but per discussion, it is not expected to be the channel that
resolves the $M_\text{Ch}$ question — it is composition- and threshold-specific (only
matters in the narrow density window around particular isotope pairs), is already
regarded as unsettled even in mainstream, fixed-$c$ astrophysics, and does not offer the
same clean, two-coupling closure that plasmon decay does. It remains a valid, real
contributor to the total cooling budget near ignition and is worth keeping on record,
but Part 4a is now the recommended focus for follow-up work, not this subsection.
...
neutrino losses. This is a genuine **crossing point** between two physically distinct
processes with different scaling behavior:
...
an approach to a fixed asymptote, a shift in the weak-rate normalization (via the
$G_F(c)$ scaling from Part 3, or an alternative choice) can shift *where the two curves
cross* by an amount that is not bounded the way Channel A's correction is. It is also
precisely the process real astrophysics
still finds difficult: even at fixed, present-day $c$, the literature describes
convective Urca physics as "not well understood," requiring dedicated 3D hydrodynamic
simulation (Denissenkov et al. 2024, MAESTROeX-based work), and separately shows SN Ia
nucleosynthetic yields are demonstrably sensitive to hand-modifications of the weak
rates (Bravo 2019).

**This document does not close this calculation.** What can be stated is the concrete
shape of what's needed:
1. The strong-sector $Q$-value scaling for the specific Urca pairs ($^{23}$Na/$^{23}$Ne,
   $^{25}$Mg/$^{25}$Na), from Part 3's principle — expected $\propto c^2$, giving a
   shifted threshold density $\rho_\text{th}(c)$ via the same relation as Channel A
   ($\rho_\text{th}\propto c^{3(n-1)}$, $n=2\Rightarrow\rho_\text{th}\propto c^3$).
...
3. **A temperature-scaling convention.** The Urca cooling rate's $T^4$ dependence
   ...
   $c$ — this is exactly the "$k_B$ convention" caveat already flagged in T18, and needs
   to be pinned down here too, most naturally via the same "energy $\propto c^2$"
   pattern used throughout this document, giving $T\propto c^2$ at fixed entropy/particle
   content (an assumption, not yet checked against the model's thermodynamics in detail).
...
works that out, and it changes the overall picture: the direction is favorable for both
T4 and T20, not neutral.
...
cooling literature states this explicitly: for ordinary-mass WDs, "the time necessary to
...
is not the duration of the phase, but the *extra energy* it removes** — because the star
...
- **Larger violations** (ratio $1.37$–$1.53$: SDSS J234044.83 at $53\%$ over,
  ...
  correction from the Gaia-recalibration issue in T20 §A to close the rest, rather than
  needing that correction to do all the work alone.
...
- **T20**: **update needed, not just a robustness check.** Part 2 shows the idealized
  $M_\text{Ch}(c)\propto c^{3/2}$ scaling used in T20's ceiling formula is safe to a few
  percent against the electron-capture/AIC correction. But Part 5's cooling-age
  correction is a substantive, favorable revision: standard cooling ages (used
  throughout T20 §A/§C) overstate true ages, plausibly enough to fully explain T20's
  marginal ceiling violations and significantly reduce the larger ones. T20's Verdict
  and Open Questions should be revised to reflect this.
...
6. Pin down the model's temperature-scaling convention ($k_BT\propto c^?$) — needed for
   both Part 4a's full closure and Part 4b, and flagged in T18 as an existing, separate
   open item.
...
6. Consider the alternative weak-sector choice (invariant Higgs vev, floating $M_W$) and
   whether it gives a distinguishable prediction anywhere before committing to the
   $G_F$-invariant choice made here.
7. Extend Part 3's strong-sector treatment beyond the Coulomb term: work out whether a
...
8. Revisit the AIC real-mass numbers ($1.37$–$1.48\,M_\odot$ across different studies) to
   pin down a more precise present-day $g(x_c)$ coefficient for Part 2's bound.
9. Once the heating-rate side (item 3) is derived, redo T4's Pantheon+ fit with the
