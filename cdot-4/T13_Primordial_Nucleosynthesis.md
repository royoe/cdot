...
Under invariant G ($G \propto c^0$, cdot-4), $G_\text{BBN} = G_0$ — the extreme
G-amplification predicted by $G \propto c^{-2}$ does not occur. The BBN G-variation
constraint (listed as a major tension in cdot-3) is resolved at root by the
invariant-G adoption (T8).
...
\quad \text{(using } c \propto R^n \text{ and invariant } G\text{)}.$$
Therefore $r_s/R \propto R^{2-2n}$.

*Note: Under cdot-3's $G \propto c^{-2}$, the exponent was $2-4n$. Under invariant G
(cdot-4), the exponent is $2-2n$.*

- For volume law $n=3$: exponent $= 2-6 = -4$ (still $\to\infty$ as $R\to 0$ — Reading 2
  survives).
- For surface law $n=2$: exponent $= 2-4 = -2$ (still $\to\infty$ as $R\to 0$ — Reading 2
  survives).
- For S′ $n=2/3$: exponent $= 2 - 4/3 = 2/3$ (positive, so $r_s/R \to 0$ as $R\to 0$;
  S′ does **not** have a super-Schwarzschild early universe under invariant G and loses
  the PBH genesis argument).

Under invariant G, the condition for a super-Schwarzschild early universe requires $n > 1$,
which is satisfied by the volume and surface laws but **not** by S′ ($n=2/3$). The PBH
genesis argument therefore applies to the volume and surface laws but fails for S′ in
the cdot-4 framework.
...
PBH-formation mechanism. Under invariant G, this crossover is **generic to the volume
and surface laws** (both have $n > 1$; both cross $r_s/R \sim 1$), so PBH formation
does **not** require S′. The volume law can simultaneously have the mildest $q_0$
tension (T4) and PBH formation. S′, however, no longer has a super-Schwarzschild early
universe under invariant G and does not participate in this mechanism.
