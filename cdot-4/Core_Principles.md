...
The model is a flat, coordinate-static cosmology. All cosmological dynamics are
carried by a time-varying speed of light $c(t)$. Four premises define it:
...
3. **Invariant rest mass and G.** Particle rest mass $m \propto c^{0}$ and Newton's
   gravitational constant $G \propto c^{0}$ are both local invariants, independent of
   the cosmological vacuum state. These are empirical premises, not derived from the
   relational principle. See T4 (mass) and T8 (G).
...
the isotropic vacuum); with invariant $G$, the orbit radius is then also conserved
($r = L^2/(m^2 GM) = \text{const}$). See T11 for the full discussion.
