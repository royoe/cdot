...
rotation-curve normalization, the origin of the mass law and
...
Its firm distinctive results: a finite proper age ($\approx21$ Gyr), the squared
...
invariant — **and not just classically: to all orders.** Every atomic length (Bohr
radius, Compton wavelength) scales as $c^{-1}$ in lockstep, so every dimensionless QED
quantity — including the vacuum-polarization (Uehling) correction, which depends on the
ratio $a_0/\lambda_C$ — is $c$-invariant. The model therefore predicts **zero**
$\dot\alpha$ (consistent with ESPRESSO, $\Delta\alpha/\alpha=1.3\pm1.3$ ppm), and the
old "inconsistent magnitudes" reflected computing a quantity that is structurally zero.
*This rests on $\epsilon_0\propto c^{-1}$ being exact and on invariant mass (premise 3):
under the PV mass branch the $a_0/\lambda_C$ lockstep breaks and $\alpha$ could drift —
so $\alpha$-invariance is one of the results favoring invariant mass (OP-4).*
...
- **The quantity $\dfrac{1}{2kR_0^{2}}$ is a timescale, not an age.** It equals
  ...
  $\tau_\text{total} = \tfrac{3}{4}(H_0^{\text{hor}})^{-1} = \tfrac{3}{2}(H_0^{\text{obs}})^{-1} \approx 21$ Gyr
  (Open Problems OP-1, general result $\tfrac{3}{3p-2}(H_0^{\text{hor}})^{-1}$).
  **Convergence domain:** this finite result requires $3p-2>0$, i.e. $p>2/3$
  ($s>-4/3$). Invariant mass ($p=2$) is comfortably inside; the **PV-mass branch**
  ($s=-3/2$, $p=1/2$) lies *outside* — its integrand $(1+\dots)^{-3/4}$ has exponent
  $>-1$, so the proper age **diverges (infinite)**, and the formula's apparent
  $3/(3p-2)=-6$ is the negative value that signals that divergence, not a real age.
  So the two mass branches differ physically in the age of the universe itself
  (finite vs infinite atomic past), not merely in a number.
  **Note the two-$H_0$ care:** the $3/4$ coefficient multiplies the *horizon* rate
  $H_0^{\text{hor}}=3kR_0^2$; converting to the observable $H_0^{\text{obs}}=PH_0^{\text{hor}}=2H_0^{\text{hor}}$
  gives $3/(2H_0^{\text{obs}})$. (An earlier draft wrote $3/(4H_0)$ and substituted
  $H_0^{\text{obs}}=70$, which halved the age to a spurious 10.5 Gyr — the same
  two-$H_0$ slip corrected for distances in OP-11.)
  Earlier drafts that called $3/(2H_0)$ a "metric age" were in error.
  **Constant-density caveat (number-Machian):** this $\approx21$ Gyr uses the volume
  law $c\propto R^3$, i.e. *constant number density* — every comoving volume holds a
  fixed count at all epochs. That is the idealization, not the true number-Machian
  $N(R)$, which changes through the radiation era, pair epochs, and baryogenesis. The
  age integral is weighted to *recent* epochs (75% accrues by $z\approx7$, 99% by
  recombination, because early atomic clocks ran slow, $\nu\propto c^2$), so the
  recent-end approximation is excellent. But if the early count was *enhanced*
  (relativistic census, OP-16 count branch), $c$ was larger early, clocks faster, and
  the age *increases*. This is **bounded**, not divergent: since $c/c_0\le1$ over a
  finite $R$, the age lies in $[21,\ \sim84]$ Gyr, the ceiling being the $c\equiv c_0$
  limit (4×). So $\approx21$ Gyr is a **lower bound** (the rest-mass branch keeps it
  near 21; the count branch pushes it higher — OP-16 again).
...
$$\text{Proper lookback time } (P=2):\quad
 \tau(z) = \frac{3}{2H_0^{\text{obs}}}\left[1 - (1+z)^{-2/3}\right],$$
with total proper age $\tau_\infty = \tfrac{3}{2} (H_0^{\text{obs}})^{-1} \approx 21$
Gyr (§OP-1). *(The prefactor is $3/(4H_0^{\text{hor}})=3/(2H_0^{\text{obs}})$ for $P=2$;
writing it as $3/(4H_0^{\text{obs}})$ was a two-$H_0$ slip that halved every time value.)*
The coordinate lookback is
...
|------:|------:|------:|------:|------:|------:|------:|------:|------:|------:|------:|
| 0.1 | 0.9535 | 1.049 | 405 | 445 | 368 | 1.29 | 460 | 380 | 1.30 | $-0.07$ |
| 0.5 | 0.8165 | 1.225 | 1679 | 2519 | 1119 | 4.96 | 2833 | 1259 | 5.04 | $-0.26$ |
| 1.0 | 0.7071 | 1.414 | 2804 | 5607 | 1402 | 7.75 | 6608 | 1652 | 7.72 | $-0.36$ |
| 2.0 | 0.5774 | 1.732 | 4299 | 12898 | 1433 | 10.88 | 15540 | 1727 | 10.24 | $-0.40$ |
| 5.0 | 0.4082 | 2.449 | 6634 | 39803 | 1106 | 14.61 | 46652 | 1296 | 12.31 | $-0.34$ |
| 10.0 | 0.3015 | 3.317 | 8466 | 93121 | 770 | 16.72 | 103843 | 858 | 13.00 | $-0.24$ |
...
- **Lookback/age:** the model's total proper age is $\approx 21$ Gyr vs
  $\Lambda$CDM's $\approx 13.8$ Gyr — the model runs **older** (by $\sim50\%$), not
  younger. This *removes* any conflict with the oldest stars and globular clusters
  ($\sim13$ Gyr); the residual question is the reverse — a universe older than the
  standard value. (The earlier "$\approx10.5$ Gyr, too young" reading was a two-$H_0$
  arithmetic slip, now corrected; OP-1.) Note the per-$z$ lookback can exceed
  $\Lambda$CDM's at high $z$ (e.g. $z=5$: 14.6 vs 12.3 Gyr), consistent with the
  larger total age.
...
finite proper age $\tfrac{3}{4}(H_0^{\text{hor}})^{-1}=\tfrac{3}{2}(H_0^{\text{obs}})^{-1}$ (§3), the photon-energy factor in luminosity
...
| Time frames + proper age $\tau = \tfrac{3}{2}(H_0^{\text{obs}})^{-1} \approx 21$ Gyr | Resolved (OP-1); corrected from a two-$H_0$ slip |
...
| Fine-structure drift $\dot\alpha$ | **Zero** — $\alpha$ invariant to all orders (OP-4); tied to invariant mass |
...
unsolved** (the retardation mechanism was demoted, OP-3; the connecton foam-sea of
OP-14 gives Newtonian $1/r$ but not the MOND $\sqrt M$ — every mechanism tried gives
Newton, so the model most likely needs dark matter for galaxies, plausibly the PBH
population that also supplies the CMB wells, OP-12/OP-18). *This
document was given a surgical update reflecting Sessions 3–5 (conservation laws, the
relational principle, the OP-3 demotion, the $q_0$ result, the OP-14 gravity findings,
and $\alpha$ shown invariant to all orders); a full reorganization around the relational
principle of §0 is flagged as a future task.*
